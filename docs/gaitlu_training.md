...
Record `study_pools.json`, `training_registry.csv`, and the exact training commit in the
private study ledger.
...
is rejected. To resume an interrupted run manually, invoke
...
Health&Gait feature archives, verify checkpoint sidecars, build the private evaluation
registry, and run the frozen `cody-jepa-gfc-study preflight` from the analysis tag. That
preflight validates the final step and the public `study_metadata` agreement, but the
current evaluator does not independently compare the private GaitLU manifest-pair digest
inside each checkpoint data contract. Audit and record those digests separately before
unblinding, or add that check before the analysis freeze.
...
the GFC-v2 preflight described in [the README](../README.md#research-workflow).
