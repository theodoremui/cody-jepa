...
from .frames import list_frame_paths, resolve_frame_dir
from .schema import REQUIRED_MANIFEST_COLUMNS


MOTION_OUTPUT_STEM = "healthgait_motion_diagnostics"
...
def summarize_healthgait_manifest(
    ...
    with manifest_csv.open(newline="") as manifest_file:
        ...
        for row in reader:
            ...
            speed_counts[row["speed"].strip()] += 1
            clothing_counts[row["clothing"].strip()] += 1
            direction_counts[row["direction"].strip()] += 1
            source_video_ids.add(row["source_video_id"].strip())
            ...
            frame_dir = resolve_frame_dir(row["frame_dir"].strip(), repo_root, resolve=False)
    ...
    return {
        ...
        "clothing_counts": dict(sorted(clothing_counts.items())),
...
def _sample_indices(split, dataset_length, sample_count, seed, epoch):
    ...
    split_seed = int.from_bytes(key, "big", signed=False)
...
def run_healthgait_motion_diagnostics(
    ...
    csv_path = output_dir / f"{MOTION_OUTPUT_STEM}.csv"
    json_path = output_dir / f"{MOTION_OUTPUT_STEM}.json"
