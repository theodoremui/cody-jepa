...
def scientific_fields(recording_id, frame_count, fps=30.0):
    return {
        "recording_id": recording_id,
        "source_video_id": f"{recording_id}::source",
        ...
        "shortcut_duration_seconds": str(frame_count / fps),
...
class HealthGaitDatasetSeedTest(unittest.TestCase):
    ...
    def _valid_row(self, root, split, subject_id, frame_count=6, num_frames=None):
        ...
        declared = frame_count if num_frames is None else num_frames
        return {
            ...
            "num_frames": str(declared),
            "split": split,
            **scientific_fields(str(relative_dir), declared),
        }
    ...
    def _write_fixture(self, root):
        ...
        for idx, split in enumerate(["train", "train", "train", "val"]):
            ...
            rows.append({
                ...
                "split": split,
                **scientific_fields(str(frame_dir.relative_to(root)), frame_count),
            })
    ...
    def test_sample_contains_traceable_clip_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(sample["sequence_id"], "frames/train/sample_0")
            ...
            self.assertEqual(sample["trial"], "WoJ_1")
    ...
    def test_factory_revalidates_cached_samples_for_evaluation_window_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            train = self._valid_row(root, "train", "P000", frame_count=8)
            val = self._valid_row(root, "val", "P001", frame_count=8)
            val_dir = root / val["frame_dir"]
            for source, target in zip(range(5, 9), range(100, 104)):
                (val_dir / f"{source:03d}.png").rename(
                    val_dir / f"{target:03d}.png"
                )
            ...
            with self.assertRaisesRegex(
                ManifestValidationError, "only 2 distinct valid windows; 3 were requested"
            ):
                build_healthgait_datasets_from_config(config)
    ...
    def test_manifest_validation_requires_existing_frame_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bad_row = {
                ...
                "num_frames": "6",
                "split": "train",
                **scientific_fields("frames/train/missing", 6),
            }
            val_row = self._valid_row(root, "val", "P001")
    ...
    def test_deterministic_window_shortage_fails_during_construction(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            train = self._valid_row(root, "train", "P000", frame_count=8)
            frame_dir = root / train["frame_dir"]
            (frame_dir / "005.png").rename(frame_dir / "100.png")
            (frame_dir / "006.png").rename(frame_dir / "101.png")
            (frame_dir / "007.png").rename(frame_dir / "102.png")
            (frame_dir / "008.png").rename(frame_dir / "103.png")
            val = self._valid_row(root, "val", "P001")
            manifest = self._write_manifest(root, [train, val])
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for split, subject in (("train", "P000"), ("val", "P001")):
                ...
                rows.append({
                    ...
                    "num_frames": "6",
                    "split": split,
                    **scientific_fields(str(relative), 6),
                })
    ...
    def test_multiple_deterministic_windows_must_have_distinct_starts(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            rows = [
                self._valid_row(root, "train", "P000", frame_count=6),
                self._valid_row(root, "val", "P001", frame_count=4),
            ]
            manifest = self._write_manifest(root, rows)
            with self.assertRaisesRegex(
                ManifestValidationError, "only 1 distinct valid windows"
            ):
                HealthGaitManifestDataset(
                    manifest,
                    split="val",
    ...
    def test_dataset_description_records_transform_and_counts(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            manifest = self._write_fixture(root)
            ...
            self.assertEqual(plain.description()["sequence_count"], 3)
            self.assertEqual(plain.description()["crop_scale"], [1.0, 1.0])
            self.assertEqual(augmented.description()["crop_scale"], [0.9, 1.0])
...
class ManifestBuilderTest(unittest.TestCase):
    def test_shortcuts_use_every_frame_and_explicit_fps(self):
        with tempfile.TemporaryDirectory() as tmp:
            frame_dir = Path(tmp)
            paths = []
            for frame_index, column in enumerate(range(4), start=1):
                pixels = np.zeros((4, 4), dtype=np.uint8)
                pixels[:frame_index, column] = 255
                path = frame_dir / f"{frame_index:03d}.png"
                Image.fromarray(pixels, mode="L").save(path)
                paths.append(path)

            shortcuts = compute_shortcut_features(
                paths, fps=10.0, foreground_threshold=0.5
            )

            self.assertAlmostEqual(shortcuts["shortcut_log_frame_count"], math.log(4))
            self.assertAlmostEqual(shortcuts["shortcut_duration_seconds"], 0.4)
            self.assertAlmostEqual(
                shortcuts["shortcut_horizontal_centroid_drift_signed"], 1.0
            )
            self.assertAlmostEqual(
                shortcuts["shortcut_horizontal_centroid_drift_absolute"], 1.0
            )
    ...
    def test_paired_directions_share_source_video_id(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            raw_root = repo_root / "data" / "healthgait" / "raw" / "Health_Gait"
            for subject in ("PA001", "PA002"):
                for trial in ("WoJ_1", "WoJ_2"):
                    frame_dir = raw_root / "silhouette" / subject / "FGS" / trial
                    frame_dir.mkdir(parents=True)
                    for index in range(4):
                        Image.new("L", (4, 4), color=255).save(
                            frame_dir / f"{index + 1:03d}.png"
                        )
            rows = build_rows(SimpleNamespace(
                raw_root=raw_root,
                modality="silhouette",
                clip_length=2,
                fps=30.0,
            ...
            subject_rows = [row for row in rows if row["subject_id"] == "PA001"]
            self.assertEqual(len(subject_rows), 2)
            self.assertEqual(
                {row["source_video_id"] for row in subject_rows}, {"PA001/FGS/WoJ"}
            )
            self.assertEqual(
                {row["direction_clip_id"] for row in subject_rows},
                {"PA001/FGS/WoJ_1", "PA001/FGS/WoJ_2"},
            )
            self.assertEqual(
                {row["direction"] for row in subject_rows}, {"R2L", "L2R"}
            )
            ...
            with manifest.open("w", newline="") as handle:
                ...
                writer.writeheader()
                writer.writerows(rows)
            train = HealthGaitManifestDataset(
                manifest, "train", repo_root, clip_length=2, image_size=(4, 4)
            )
            val = HealthGaitManifestDataset(
                manifest, "val", repo_root, clip_length=2, image_size=(4, 4)
            )
            self.assertEqual(len(train.samples), 2)
            self.assertEqual(len(val.samples), 2)

    def test_builder_skips_recordings_without_three_contiguous_windows(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            raw_root = repo_root / "data" / "healthgait" / "raw" / "Health_Gait"
            for subject in ("PA001", "PA002"):
                for trial in ("WoJ_1", "WoJ_2"):
                    ...
                    frame_dir.mkdir(parents=True)
                    indices = range(1, 7)
                    if subject == "PA001" and trial == "WoJ_2":
                        indices = (1, 2, 3, 4, 100, 101)
                    for index in indices:
                        Image.new("L", (4, 4), color=255).save(
                            frame_dir / f"{index:03d}.png"
                        )
            ...
            self.assertNotIn("PA001/FGS/WoJ_2", {row["recording_id"] for row in rows})
            self.assertEqual(len(rows), 3)

    def test_builder_skips_recordings_with_duplicate_numeric_frame_indices(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            raw_root = repo_root / "data" / "healthgait" / "raw" / "Health_Gait"
            for subject in ("PA001", "PA002", "PA003"):
                frame_dir = raw_root / "silhouette" / subject / "FGS" / "WoJ_1"
                frame_dir.mkdir(parents=True)
                ...
                if subject == "PA001":
                    Image.new("L", (4, 4), color=255).save(frame_dir / "001.jpg")
            ...
            self.assertEqual(
                ...
                {"PA002/FGS/WoJ_1", "PA003/FGS/WoJ_1"},
            )

    def test_builder_preserves_a_training_subject_for_high_validation_fraction(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            raw_root = repo_root / "data" / "healthgait" / "raw" / "Health_Gait"
            for subject in ("PA001", "PA002"):
