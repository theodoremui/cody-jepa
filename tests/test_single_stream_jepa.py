...
class SingleStreamJEPATest(unittest.TestCase):
    ...
    def test_epoch_boundary_checkpoint_resume_matches_uninterrupted_training(self):
        ...
        semantic_keys = {
            ...
            "train_cosine", "train_examples", "train_variance_loss",
            "train_covariance_loss", "train_clip_variance_loss",
            "train_clip_covariance_loss", "val", "train_eval",
        }
        self.assertEqual(
    ...
    def test_atomic_latest_and_best_checkpoints_are_written(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(latest["completed_epochs"], best["completed_epochs"])
            for name, tensor in latest["target_encoder"].items():
                self.assertTrue(torch.equal(tensor, best["target_encoder"][name]))
...
class AntiCollapseSafeguardTest(unittest.TestCase):
    def test_vicreg_flags_collapsed_and_redundant_features(self):
        ...
        variance_loss, covariance_loss = vicreg(collapsed)
        ...
        variance_loss, covariance_loss = vicreg(redundant)
    ...
    def test_vicreg_is_small_for_healthy_features(self):
        ...
        variance_loss, covariance_loss = vicreg(healthy)
        self.assertLess(float(variance_loss), 0.01)
    ...
    def test_vicreg_flattens_token_features_and_validates_inputs(self):
        ...
        from_tokens = vicreg(tokens)
        from_flat = vicreg(tokens.reshape(-1, 6))
        self.assertAlmostEqual(
    ...
    def test_invalid_safeguard_config_fails_before_training(self):
        for key, value, message in (
            ...
            ("clip_var_coef", -0.1, "clip_var_coef"),
            ("clip_cov_coef", float("nan"), "clip_cov_coef"),
    ...
    def test_clip_vicreg_runs_logs_and_changes_encoder_updates(self):
        ...
        baseline = train_jepa(
            baseline_cfg, baseline_train, baseline_val, {"dataset": "clip"}, device="cpu"
        )
        clip_cfg = tiny_config(num_epochs=1)
        clip_cfg.update({"clip_var_coef": 1.0, "clip_cov_coef": 0.0})
        clip_train, clip_val = tiny_loaders()
        clipped = train_jepa(
            clip_cfg, clip_train, clip_val, {"dataset": "clip"}, device="cpu"
        )
        metrics = clipped["history"][0]
        self.assertIsNone(metrics["train_variance_loss"])
        self.assertIsNone(metrics["train_covariance_loss"])
        self.assertTrue(math.isfinite(metrics["train_clip_variance_loss"]))
        self.assertTrue(math.isfinite(metrics["train_clip_covariance_loss"]))
        self.assertGreaterEqual(metrics["train_clip_variance_loss"], 0.0)
        self.assertTrue(any(
            not torch.equal(left, right)
            for left, right in zip(
                baseline["context_encoder"].parameters(),
                clipped["context_encoder"].parameters(),
            )
        ))
        self.assertEqual(clipped["checkpoint_state"]["config"]["clip_var_coef"], 1.0)
        self.assertEqual(clipped["checkpoint_state"]["config"]["clip_cov_coef"], 0.0)

    def test_explicit_zero_clip_vicreg_preserves_baseline_trajectory(self):
        absent_cfg = tiny_config(num_epochs=1)
        absent_train, absent_val = tiny_loaders()
        absent = train_jepa(
            absent_cfg, absent_train, absent_val, {"dataset": "zero"}, device="cpu"
        )
        zero_cfg = tiny_config(num_epochs=1)
        zero_cfg.update({"clip_var_coef": 0.0, "clip_cov_coef": 0.0})
        zero_train, zero_val = tiny_loaders()
        explicit = train_jepa(
            zero_cfg, zero_train, zero_val, {"dataset": "zero"}, device="cpu"
        )
        for module in ("context_encoder", "target_encoder", "predictor"):
            self.assertTrue(all(
                torch.equal(left, right)
                for left, right in zip(
                    absent[module].parameters(), explicit[module].parameters()
                )
            ))
        ignored = {"epoch_seconds", "examples_per_second"}
        self.assertEqual(
            [{key: value for key, value in row.items() if key not in ignored} for row in absent["history"]],
            [{key: value for key, value in row.items() if key not in ignored} for row in explicit["history"]],
        )

    def test_resume_treats_absent_clip_coefficients_as_zero_only(self):
        cfg = tiny_config(num_epochs=1)
        train, val = tiny_loaders()
        result = train_jepa(cfg, train, val, {"dataset": "resume"}, device="cpu")
        current = tiny_config(num_epochs=2)
        current.update({"clip_var_coef": 0.0, "clip_cov_coef": 0.0})
        validate_resume_state(
            result["checkpoint_state"], current, DEFAULT_MASK_GROUPS, {"dataset": "resume"}
        )
        current["clip_var_coef"] = 1.0
        with self.assertRaisesRegex(ValueError, "clip_var_coef"):
            validate_resume_state(
                result["checkpoint_state"], current, DEFAULT_MASK_GROUPS, {"dataset": "resume"}
            )

    def test_clip_vicreg_rejects_batch_size_one_before_training(self):
        cfg = tiny_config(num_epochs=1)
        cfg.update({"batch_size": 1, "clip_var_coef": 1.0})
        train = DataLoader(TinyVideoDataset("train", 2, cfg["seed"]), batch_size=1)
        _, val = tiny_loaders()
        with self.assertRaisesRegex(ValueError, "batch_size >= 2"):
            train_jepa(cfg, train, val, {"dataset": "batch-one"}, device="cpu")
